...
public class QGame {
	...
	public static void diffE() {
		...
		for (int i = 1; i < 6; i++) {
			System.out.println("here is riddle #"+ i + " " + eRiddles[i-1] + "\n\n what is your answer?");
			userAnswer[i-1] = sc.nextLine().toLowerCase();
			if(userAnswer[i-1].contains(eAnswer[i-1])) {
	...
	public static void diffM() {
		...
		for (int i = 1; i < 6; i++) {
			System.out.println("here is riddle #"+ i + mRiddles[i-1] + "\n\n what is your answer?");
			userAnswer[i-1] = sc.nextLine().toLowerCase();
			if(userAnswer[i-1].contains(mAnswer[i-1])) {
				System.out.println("CORRECT, + 1 POINT");
				score++;
			}
			else {
